...
def novo_jogo():
    ...
    for linha in range(3):
        for coluna in range(3):
            botoes[linha][coluna].config(text='', bg="#275db3",bd=1,highlightbackground='#000000')
...
rotulo = customtkinter.CTkLabel(master=janela,text=jogador + " vez", font=('consolas', 40))
rotulo.pack(side='top')

#botao_reiniciar = Button(master=janela,text='Reiniciar', font=('consolas', 20), command=novo_jogo)
botao_reiniciar = customtkinter.CTkButton(master=janela,text='Reiniciar', font=('consolas', 20), command=novo_jogo)
